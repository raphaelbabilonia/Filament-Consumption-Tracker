...
def update_database_schema():
    ...
    if not db_files:
        print(f"Error: No database files found in {APP_FOLDER}")
        return False
    
    # Print the list of database files and let the user select one
    print("\nAvailable database files:")
    for i, file in enumerate(db_files):
        print(f"{i+1}. {file}")
    
    # Choose the backup file to use
    print("\nWhich database file would you like to use as the source? (enter the number)")
    try:
        choice = int(input("> "))
        ...
        source_db_path = os.path.join(APP_FOLDER, db_files[choice-1])
        print(f"Using {source_db_path} as the source database")
    except ValueError:
